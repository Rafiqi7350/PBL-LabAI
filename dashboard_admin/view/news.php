...
$result = pg_query($conn, "SELECT * FROM view_news_detail ORDER BY id ASC");
?>
...
<head>
    ...
    <style>
        ...
        .sidebar { width: 240px; height: 100vh; background-color: #fff; position: fixed; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
        .sidebar .nav-link { color: #444; font-weight: 500; margin-bottom: 6px; }
        .sidebar .nav-link.active { background-color: #ff8c00; color: #fff; }
        .topbar { height: 60px; background-color: #0b3c75; color: white; display: flex; justify-content: space-between; align-items: center; padding: 0 25px; }
        .content { margin-left: 240px; padding: 25px; }
        .search-box { width: 280px; border-radius: 50px; padding-left: 40px; }
        .search-icon { position: absolute; left: 15px; top: 10px; color: gray; }
...
<div class="content">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="fw-semibold mb-0">News List</h5>

        <div class="d-flex align-items-center gap-3">
            <div class="position-relative">
                <i class="bi bi-search search-icon"></i>
                <input type="text" id="searchInput" class="form-control search-box" placeholder="Search News">
            </div>
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr class="news-row"
                        ...
                        data-isi="<?= strtolower($row['isi']) ?>">

                        <td><?= $no++ ?></td>

                        <td>
                            <img src="../assets/img/news/<?= htmlspecialchars($row['gambar']) ?>"
                        ...
                        <td>
                            ...
                            <a href="../controller/delete_news.php?id=<?= $row['id'] ?>"
            ...
            <form action="../controller/update_news.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="text-center mb-3">
                        <img id="preview_gambar" src="" width="90" height="90" class="rounded border shadow-sm">
                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label">Isi</label>
                        <textarea name="isi" id="edit_isi" class="form-control" rows="4"></textarea>
                    </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    editButtons.forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            document.getElementById("edit_tanggal").value = btn.dataset.tanggal.replace(' ', 'T');

            preview.src = "../assets/img/news/" + btn.dataset.gambar;

            fileInput.value = "";
        });
    });

    // Preview gambar baru saat ganti
...
document.getElementById("searchInput").addEventListener("input", function () {
    ...
    rows.forEach(row => {
        ...
        const isi = row.dataset.isi;

        row.style.display = (judul.includes(keyword) || isi.includes(keyword)) ? "" : "none";
    });
});
</script>
